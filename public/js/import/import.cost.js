$(function () {
    ...
    $("#importFileButton").click(function (){
        ...
        $.ajax({
            method: "POST",  
            url: "/cost/project_cost/check_file_name",
            dataType: "json",
            data:{
                    ...
                    $.ajax({
                        ...
                        success: function(message) {
                            switch(message['switch']) {
                                ...
                                case '8':
                                    $('#errorMessage').removeClass('hide').addClass('show');
                                    $('#errorMessage #message').text('Please select a specific Team before importing!');
                                    break;    
                            }
                        },
                        contentType: false,
    ...
    $("#confirmButton").click(function (){
        ...
        var uploadfile = new FormData($("#formImport")[0]);
        $.ajax({
            url: '/cost/project_cost/import_after_confirm',
            type: 'POST',
            data: uploadfile,
            dataType: "json",
            beforeSend: function(){
            ...
            success: function(message) {
                switch(message['switch']) {
                    ...
                    case '6':
                        ...
                        break;
                    case '7':
                        console.log(message['importExcel']);
                        console.log(message);
                        $('#ticketID').val(message['importExcel']);
                        $('#exportFileAfterImport').modal({
                            backdrop: 'static',
