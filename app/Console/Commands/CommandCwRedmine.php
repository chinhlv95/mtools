...
/**
 *
 * Nov 30, 2016 2:48:05 PM
 * @author tampt6722
 *
 */
class CommandCwRedmine extends Command {
    ...
    public function __construct(
        ...
        $this->client = new Client(
                env('COWELL_REDMINE_URL', ''),
                env('COWELL_REDMINE_API_KEY', ''));
    ...
    */
                            ...
                            if (count($existedTicket) == 0) {
                                ...
                                if (!empty($ticket ['status'])) {
                                    ...
                                    $dataTicket ['status_id'] =  $this->statusRepository
                                            ->getStatusId($integratedStatusId, $statusName, 4);
                                }
                                ...
                                if (!empty($ticket['priority'])) {
                                    $dataTicket ['priority_id'] = $this->priorityRepository
                                                ->getPriorityId($ticket['priority'], 4);
                                }
                                ...
                                if (!empty($ticket['custom_fields'])) {
                                    ...
                                    if (!empty($cusFields['impact_analysis'])) {
                                        $dataTicket['impact_analysis'] = $cusFields['impact_analysis'];
                                    }
                                }
                                ...
                                if (!empty($ticket ['tracker'])) {
                                    $dataTicket ['ticket_type_id'] = $this->ticketTypeRepository
                                            ->getTicketTypeId( $ticket ['tracker'] ['id'],
                                                    $ticket ['tracker'] ['name'], 4);
                                }

                                $ticketId = $this->ticketRepository->save($dataTicket);
                                $this->locRepository->saveloc($projectId, $ticketId, $pUserId,
                                        $loc, $integratedCreatedAt, $integratedUpdatedAt);
                                $this->crawlerUrlRepository->saveToCrawlerUrls($crawlerTypeId,
                                        $ticketId, $ticket, 'Ticket Cowell Redmine', 'issue');
                            }
                            $this->adjuster->adjust(1);
                        } catch (Exception $e) {
                            $crawlerErr = [];
    ...
    protected function getCustomFields($ticketCusFields){
        ...
        foreach ($ticketCusFields as $customFields) {
            ...
            if ($customFields['name'] == 'Number of Test case' || $customFields['name'] == 'Test case ID') {
                if (!empty($customFields['value'])) {
                    $dataTicket['test_case'] =  $customFields['value'];
                }
            }
            if ($customFields['name'] == 'Who made defect') {
                if (!empty($customFields['value'])) {
                    $user = $this->saveUserInfo($customFields['value']);
                    $dataTicket['made_by_user'] = $user['user_name'];
                }
            }
            if ($customFields['name'] == 'Defect Weight') {
                if (!empty($customFields['value'])) {
                    $dataTicket['bug_weight_id'] =  $this->bugWeightRepository
                        ->getBugWeightId($customFields['value'], 4);
                }
            }

            if ($customFields['name'] == 'Defect Type') {
                if (!empty($customFields['value'])) {
                    $dataTicket['bug_type_id'] =  $this->bugTypeRepository
                        ->getBugTypeId($customFields['value'], 4);
                }
            }
            if ($customFields['name'] == 'Impact analysis') {
                if (!empty($customFields['value'])) {
                    $dataTicket['impact_analysis'] =  $customFields['value'];
                }
            }
        }
        return $dataTicket;
                ...
                if (count($tickets) > 0) {
                    foreach ($tickets as $tk) {
                        try {
                            ...
                            $integratedTicketId = $tk->integrated_ticket_id;
                            $updatedAt = $tk->integrated_updated_at;
                            $ticketUpdate = $this->client->issue
                                                    ->show($integratedTicketId);
                            if (!empty($ticketUpdate)) {
                                ...
                                $checkEntries = $this->entryRepository
                                                        ->getDataByAttribute('ticket_id', $ticketId);
