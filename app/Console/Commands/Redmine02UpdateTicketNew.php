...
/**
 * Get new ticket Redmine 02 created yesterday
 * Oct 27, 201610:10:02 AM
 * @author tampt6722
 *
 */
class Redmine02UpdateTicketNew extends CommandRedmine02
{
    /**
     ...
     * @var string
     */
    protected $signature = '02_update_new_ticket:get {crawler_type}';
    ...
    public function handle() {
        try {
            ...
            if (count($checkCrawlerType) > 0) {
                ...
                $projects = $this->projectRepository->getProjectsByAttribute('source_id', 3, 2 );
                if (count($projects) > 0) {
                    foreach($projects as $project ) {
                        try {
                            ...
                            if (!empty($crawlerUrl)) {
                                ...
                                $crawlerErr['status_code'] = app('Illuminate\Http\Exception\HttpResponseException')->getResponse();
