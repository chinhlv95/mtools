        ...
        <div class="panel-body" id="form_body">
            <form method="post" id="frm_update" action="{{ URL::route('content-management.update') }}" class="form-horizontal">
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <input type="text" readonly="readonly" name="releated_name" value="{{$data[0]['name']}}" class="form-control input-sm">
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <select class="form-control" name="related_id">
                            @foreach($related_name as $key => $value)
                                <option <?php if($value->key == $data[0]['related_id']): ?> selected="selected" <?php endif; ?> value="{{ $value->key }}">{{ $value->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
